        ...
        return Observable.of(null).mergeMap(() => {
            ...
            if (request.url.endsWith('/auth/login') && request.method === 'POST') {
                ...
                const filteredUsers = users.filter((user: User) => {
                    ...
                    if ( request.body.email === 'k40cntt@gmail.com' && request.body.password === 'nhiyeudkvn') {
            ...
            if (request.url.endsWith('/api/users') && request.method === 'GET') {
                if (request.headers.get('Authorization') === 'fake-jwt-token') {
                    ...
                    return Observable.throw(  new HttpErrorResponse({ status: 403, statusText: 'Error author' }));
                }
            }
            ...
            if (request.url.endsWith('/api/users') && request.method === 'POST') {
                ...
                // if (duplicateUser) {
                    // return Observable.throw('Email "' + newUser.email + '" Tồn tại');
                // }
